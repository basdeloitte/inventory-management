<template>
  <div class="orders">
    ...
    <div v-else>
      ...
      <!-- Submitted Orders section: only shown when restock orders exist -->
      <div v-if="restockOrders.length > 0" class="card" style="margin-top: 1.5rem">
        <div class="card-header">
          ...
          <table class="orders-table restock-orders-table">
            ...
            <tbody>
              <tr v-for="order in restockOrders" :key="order.id">
                ...
                <td class="col-date">{{ formatDate(order.order_date) }}</td>
                <td class="col-date">{{ formatDate(order.expected_delivery) }}</td>
                <td class="col-lead">{{ t('orders.leadTimeDays') }}</td>
